...
with st.container():
    ...
    with col1:
        ...
        time_to_money_conversion = st.slider(
            "Time to Money Conversion ($/hour)", 0, 50, 25
        )
        car_pc = st.slider(
            "Percentage of Population who own car (%)", 0.0, 1.0, 1 / 2, step=0.01
        )

    with col2:
        safety_cost = st.slider("Safety Cost ($)", 0, 50, 20)
        hour_of_day = st.slider("Hour of Day (5AM-21PM)", 5, 21, 17, step=1)
        inconvenience_fee = st.slider(
            "Driving Inconvenience Cost ($)", 0, 100, 20, step=1
        )
...
ub = UberBartMix(
    time_to_money_conversion, neighborhoods["distance_to_berryessa_miles"], safety_cost
)
...
neighborhoods["cost_drive"] = Drive(
    time_to_money_conversion, inconvenience_fee
).get_cost(hour_of_day, is_weekday)
